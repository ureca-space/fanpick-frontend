...
as $$
  with prediction_results as (
    select
      p.user_id,
      case
        when p.result in ('correct', 'incorrect') then p.result
        when m.status = 'finished'
          and m.score ~ '^[0-9]+:[0-9]+$'
          and split_part(m.score, ':', 1)::integer
            <> split_part(m.score, ':', 2)::integer
          then
            case
              when upper(coalesce(p.selected_team_code, '')) =
                case
                  when split_part(m.score, ':', 2)::integer
                    > split_part(m.score, ':', 1)::integer
                    then upper(coalesce(m.home_team_code, ''))
                  else upper(coalesce(m.away_team_code, ''))
                end
                then 'correct'
              else 'incorrect'
            end
